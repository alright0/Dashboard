...
import settings
from pgconn import cont_material, cont_query, cut_description, send_query, get_name
...
def get_df_lvl_0(dt, dt2):
    ...
    df_lvl_0["DateTimeStart"] = pd.to_datetime(
        df_lvl_0["DateStart"].astype(str) + " " + df_lvl_0["TimeStart"]
    )
...
def get_df_line_lvl_1(df, line):
    ...
    df4["letter"] = (
        df4["Stop Time"].astype(str).str[:10] + " " + df4["Shift"].astype(str)
    )
    ...
    df4 = pd.merge(df4, df_letter, how="inner", left_on="letter", right_on="date")
...
def shift_let_list():
    ...
    df["date"] = df["dateraw"].astype(str).str[:10] + " " + df["shift"].astype(str)
...
def make_bar(df_line_lvl_1, indicat_df, line):
    ...
    if not df_line_lvl_1.empty:
        ...
        ready_df = df_line_lvl_1[["Date", "letter", "Sheets", "Shift"]]
    ...
    fig.add_trace(
        go.Bar(
            y=table_df["Planned Output"],
            x=table_df["letter"],
            width=0.25,
            offset=-0.5,
            marker={"color": "#9c9c9c"},
            hovertext=("План: " + table_df["Planned Output"].astype(int).astype(str)),
            hoverinfo="text",
        )
    )
    ...
    fig_t = go.Figure(
        data=[
            go.Table(
                ...
                header=dict(
                    values=table_df.columns.values,
                    align=["center", "center", "center", "center", "center"],
                    fill=dict(
                        color=["#768fad", "#768fad", "#768fad", "#919191", "#919191"]
                    ),
                ),
                cells=dict(
                    values=table_df.transpose(),
                    fill=dict(
                        color=["#cde0f7", "#cde0f7", "#cde0f7", "#dedede", "#dedede"]
                    ),
...
def make_line(df, dt, dt2):
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(margin=dict(t=10, l=10, b=10, r=10)),
    )
    ...
    for code in codes:
        ...
        # фильтрация df
        df2 = df.loc[
            (df["Stop Code"] == code) & (df["TimeRaw"] > 60) & (df["Status"] == "STOP")
        ]
        ...
        fig.add_trace(
            go.Bar(
                ...
                hovertext=(
                    ...
                    + df2["name_ru"]
                    + ".<br>Stopped at "
                    + df2["Stop Start"].astype(str).str[11:]
                    + "<br>Duration: "
                    + df2["Minutes"].astype(str).str[7:15]
                ),
...
def get_df_bar_indicat(df, line):
    ...
    if df["status"].iloc[0] and shift != 0:
        ...
        label_indicator = "{} {} min".format(stopcode, df["stop_time"].iloc[0])
        # print(df_codes)
...
def make_line_table(df_line_lvl_1, dt, dt2):
    ...
    df_line_lvl_1 = df_line_lvl_1.loc[
        (df_line_lvl_1["Stop Time"] > dtst) & (df_line_lvl_1["Stop Time"] < dtst2)
    ]
    ...
    df_output_table = pd.merge(
        df_output_table, df_temp_shift, how="outer", left_on="Shift", right_on="Shift"
    )
    ...
    fig = go.Figure(
        data=[
            go.Table(
                columnwidth=[20, 80],
                header=dict(values=df_output_table.columns.values, height=30),
                cells=dict(
    ...
    df_codes_table = df_line_lvl_1.groupby(["Shift", "Stop Code"])["TimeRaw"].agg(
        ["sum", "count"]
    )
