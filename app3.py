...
plan_path = r'K:\STP\Display\Page\Plan.csv'
...
# здесь создается набор графиков и таблиц по линиям.
    ...
    plan_settings.append(
        html.Div([
            ...
            html.P(
                '{} ({}K pcs/shift): '.format(line, int(settings.LINE_OUTPUT.get(line)/1000)), 
                style={
...
@app.callback(
    [dash.dependencies.Output('{} quantity'.format(letter), 'children') 
        for letter in line_letters],
    [dash.dependencies.Output('{} sum'.format(line), 'children') 
        for line in lines],
    [dash.dependencies.Input(letter, 'value') 
        for letter in line_letters])
def quantity(*args, **kwargs):
    ...
    line_list = ['TOTAL: {}/{:,d}'.format(val,val2) for val, val2 in zip(line_dict_sh.values(),line_dict.values())]
...
@app.callback(
    [dash.dependencies.Output('{} plan value'.format(line), 'value') 
        for line in lines],
    [dash.dependencies.Output(letter, 'value') 
        for letter in line_letters],
    [dash.dependencies.Input('load','n_clicks'),
    dash.dependencies.Input('setyear','value'),
    dash.dependencies.Input('setmonth','value'),]
)
def load_plan(n_clicks, year, month):
    ...
    df_letter = df_letter.loc[(df_letter['year'] == year) & (df_letter['month'] == month)]
...
@app.callback(
    dash.dependencies.Output('plans_text','children'),
    [dash.dependencies.Input('save','n_clicks'),],
    [dash.dependencies.State('setyear','value'),
    dash.dependencies.State('setmonth','value')],
    [dash.dependencies.State('{} plan value'.format(line), 'value') 
        for line in lines],
    [dash.dependencies.State(letter, 'value') 
        for letter in line_letters],
    )
