...
def update(*args):
    ...
    for line in lines:
        ...
        (
            ...
            ln_output,
            shift,
        ) = mkdf.get_df_bar_indicat(line_info, line)
        ...
        if shift:
            ...
            fig = go.Figure(
                data=go.Scatter(y=list(graph[line]), x=list(x), name="input"),
                layout=go.Layout(
            ...
            fig.update_layout(
                ...
                annotations=[
                    {
                        "text": """<b>Input: </b> {:14,}<br><b>Output:</b> {:9,}<br>""".format(
                            ln_input, ln_output
                        ).replace(
                            ",", " "
                        ),
            ...
            fig = go.Figure(
                data=go.Scatter(x=[], y=[], name="input"),
                layout=go.Layout(
